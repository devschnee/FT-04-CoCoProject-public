...
//튜토리얼 전용 스테이지 매니저임!!!!!
public class TutorialStageManager : MonoBehaviour, IStageManager
{
    ...
    async void Start()
    {
        ...
        if (UserData.Local.passedTutorials == 0)
    ...
    IEnumerator StageStart()
    {
        ...
        //혹시 모르니, 파이어베이스매니저의 인스턴스가 현재 맵데이터를 받아올 때까지 기다리도록 함.
        yield return new WaitUntil(() => FirebaseManager.Instance.currentMapData != null && !string.IsNullOrEmpty(FirebaseManager.Instance.selectStageID));
        currentMapData = FirebaseManager.Instance.currentMapData;
        currentStageId = FirebaseManager.Instance.selectStageID;
        stageRoot.name = currentStageId;
        ...
        FirebaseManager.Instance.selectStageID = null;
    ...
    {
        UserData.Local.passedTutorials++;
        UserData.Local.Save();
